...
func GetAppRegistration(clientid string) (outputdata [3]string, err error) {
	...
	if err != nil {
		log.Println("Connection not available ", err.Error())
		err = utils.ErrDatabaseConnectionNotAvailable
		return
	}
	query := `select clientid,redirecturi,clientsecret from dbo.appreg where clientid=@clientid`
	row := db.QueryRowContext(ctx, query, sql.Named("clientid", clientid))

	err = row.Scan(&outputdata[0], &outputdata[1], &outputdata[2])
	if err != nil {
		if err == sql.ErrNoRows {
			err = utils.ErrDatabaseRecordNotFound
			return
		} else {
			err = utils.ErrDatabaseReadFailed
			return
		}
	}
